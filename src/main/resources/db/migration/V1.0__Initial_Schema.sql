-- Flyway Migration V1.0__Initial_Schema.sql
-- This is a placeholder for the initial database schema
-- Actual migrations will be created as features are implemented

-- This file ensures the migration directory structure exists
-- ============================================
...
-- Tipos ENUM para validación a nivel de BD (idempotent creation)
DO
$$
    BEGIN
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'token_state_enum') THEN
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'logout_reason_enum') THEN
...
CREATE INDEX IF NOT EXISTS idx_operation_module_id ON operation (module_id);
CREATE INDEX IF NOT EXISTS idx_operation_access_level ON operation (access_level);
...
CREATE INDEX IF NOT EXISTS idx_role_hierarchy_level ON role (hierarchy_level);
...
CREATE INDEX IF NOT EXISTS idx_user_auth_is_active ON user_auth (is_active);
...
CREATE INDEX IF NOT EXISTS idx_user_role_is_active ON user_role (is_active);
CREATE INDEX IF NOT EXISTS idx_user_role_valid_from ON user_role (valid_from);
CREATE INDEX IF NOT EXISTS idx_user_role_valid_until ON user_role (valid_until);
...
CREATE INDEX IF NOT EXISTS idx_user_session_session_token ON user_session (session_token);
...
CREATE INDEX IF NOT EXISTS idx_session_role ON session_role (session_id, role_id);
CREATE INDEX IF NOT EXISTS idx_session_role_session_id ON session_role (session_id);
CREATE INDEX IF NOT EXISTS idx_session_role_role_id ON session_role (role_id);
CREATE INDEX IF NOT EXISTS idx_session_role_is_active ON session_role (is_active);
...
CREATE INDEX IF NOT EXISTS idx_permission_operation_id ON permission (operation_id);
CREATE INDEX IF NOT EXISTS idx_permission_valid_from ON permission (valid_from);
CREATE INDEX IF NOT EXISTS idx_permission_valid_until ON permission (valid_until);
CREATE INDEX IF NOT EXISTS idx_permission_conditions ON permission USING gin (conditions);
...
CREATE TABLE IF NOT EXISTS role_activation_requirement
(
    id                         SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_role_activation_requirement_role_id ON role_activation_requirement (role_id);
...
CREATE TABLE IF NOT EXISTS role_conflict
(
    id                  SERIAL PRIMARY KEY,
    role_id             INTEGER                                 NOT NULL REFERENCES role (role_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_role_conflict_role_id ON role_conflict (role_id);
CREATE INDEX IF NOT EXISTS idx_role_conflict_conflicting_role_id ON role_conflict (conflicting_role_id);
CREATE INDEX IF NOT EXISTS idx_role_conflict_conflict_type ON role_conflict (conflict_type);
...
CREATE TABLE IF NOT EXISTS role_prerequisite
(
    id               SERIAL PRIMARY KEY,
    role_id          INTEGER                               NOT NULL REFERENCES role (role_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_role_prerequisite_role_id ON role_prerequisite (role_id);
...
CREATE INDEX IF NOT EXISTS idx_role_schedule_role_id ON role_schedule (role_id);
CREATE INDEX IF NOT EXISTS idx_role_schedule_day_of_week ON role_schedule (day_of_week);
...
CREATE INDEX IF NOT EXISTS idx_jwt_token_jti ON jwt_token (jti);
CREATE INDEX IF NOT EXISTS idx_jwt_token_user_id ON jwt_token (user_id);
CREATE INDEX IF NOT EXISTS idx_jwt_token_session_id ON jwt_token (session_id);
CREATE INDEX IF NOT EXISTS idx_jwt_token_expiry_date ON jwt_token (expiry_date);
CREATE INDEX IF NOT EXISTS idx_jwt_token_is_valid ON jwt_token (is_valid);
CREATE INDEX IF NOT EXISTS idx_jwt_token_state ON jwt_token (state);
CREATE INDEX IF NOT EXISTS idx_jwt_token_token_type ON jwt_token (token_type);
...
-- Aplicar trigger a todas las tablas con updated_at (idempotent)
DROP TRIGGER IF EXISTS trigger_module_updated_at ON module;
CREATE TRIGGER trigger_module_updated_at
    BEFORE UPDATE
    ON module
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS trigger_operation_updated_at ON operation;
CREATE TRIGGER trigger_operation_updated_at
    BEFORE UPDATE
    ON operation
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS trigger_role_activation_requirement_updated_at ON role_activation_requirement;
CREATE TRIGGER trigger_role_activation_requirement_updated_at
    BEFORE UPDATE
    ON role_activation_requirement
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS trigger_user_auth_updated_at ON user_auth;
...
DROP TRIGGER IF EXISTS trigger_jwt_token_updated_at ON jwt_token;
CREATE TRIGGER trigger_jwt_token_updated_at
    BEFORE UPDATE
    ON jwt_token
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS trigger_validate_role_cardinality ON user_role;
CREATE TRIGGER trigger_validate_role_cardinality
    BEFORE INSERT OR UPDATE
    ON user_role
    FOR EACH ROW
EXECUTE FUNCTION validate_role_cardinality();
...
DROP TRIGGER IF EXISTS trigger_validate_role_conflicts ON user_role;
CREATE TRIGGER trigger_validate_role_conflicts
    BEFORE INSERT OR UPDATE
    ON user_role
    FOR EACH ROW
EXECUTE FUNCTION validate_role_conflicts();
...
DROP TRIGGER IF EXISTS trigger_update_session_activity ON session_role;
